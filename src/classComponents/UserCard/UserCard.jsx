...
export default function UserCard({ user, aula }) {
    ...
    return (
        <div onClick={handleClick} className="user-card">
            <img src={user.img_url} alt={`${user.nombre}`} className="user-card-image" />
            <div className="user-card-info">
                <h3 className="user-card-name">{user.nombre}</h3>
                <p className="user-card-email">{user.email}</p>
                <p className="user-card-password">********</p>
            </div>
            <div className='mini-user-card-svg-container edit-btn-top-t' >
